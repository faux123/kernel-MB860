 ...
 */
static ssize_t write_abs(struct device *dev, struct device_attribute *attr, const char *buffer, size_t count)
{
    int c, x, y, b;
    ...
    {
        /*
         * KEY_ESC happens to be 1. This is what I get for trying to squeeze
         * this into the original keyless design without adapting it properly.
         */
...
int __init aev_init(void)
{
    ...
    if(IS_ERR(aev_dev))
    {
        printk ("aev_init: error\n");
        return(PTR_ERR(aev_dev));
    }
   
    sysfs_create_group(&aev_dev->dev.kobj, &abs_attr_group);
    ...
    */
    ...
    input_register_device(aev_abs_dev);

    printk("AEV initialized\n");
  
    return(0);
}

void aev_cleanup(void)
{
