...
TmonGetInstance(NvOdmTmonZoneID ZoneId)
{
    ...
    if (!s_Initialized)
    {
        NvOdmOsMemset(&s_Tmon, 0, sizeof(s_Tmon));
        ...
        if (pConn)
        {
            ...
            pTmon->pfnTherm2Asserted    = Adt7461Therm2Asserted;
    ...
    NvOdmTmonDeviceHandle hTmon)
{
    ...
    if (pTmon && pTmon->RefCount)
    {
        NV_ASSERT(pTmon->pfnTherm2Asserted);
        return pTmon->pfnTherm2Asserted(pTmon);
    }
