...
#define ADT7461_ODM_INTR_POLARITY (NvOdmGpioPinMode_InputInterruptAny)
...
// THERM1 output is dedicated for critical h/w shutdown
// ALERT/THERM2 output is configured as THERM2.  It cannot be configured as ALERT, because the part
// is SMBUS compliant then and responds to the I2C address of magnetometer (0x0C) when it has an 
// unclreared iterrupt causing I2C bus contention and holding SDI line low
#define ADT7461_INITIAL_CONFIG \
        ((ADT7461_ODM_EXTENDED_RANGE  ? ADT7461ConfigBits_ExtendedRange : 0)    | \
         (ADT7461ConfigBits_IntrAutoClear))
...
// Temperature readings averaging is used to reduce the effects of noise. Averaging is implemented 
// only for conversations rates of 16 or less
#define ADT7461_INITIAL_RATE_SETTING        (0x08)

// ADT7461 I2C (SMBus) clock speed, bus timeout, retries`
