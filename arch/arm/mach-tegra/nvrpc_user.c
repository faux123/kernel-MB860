...
{
	int *pMem = kzalloc(sizeof(int), GFP_KERNEL);

	if (!pMem) return -ENOMEM;
	file->private_data = pMem;
	return 0;
}

int nvrpc_close(struct inode *inode, struct file *file)
{
	int *pMem = (int *)file->private_data;
	...
	if ( (*pMem) ) {
		...
		init_ref_count--;
		if (init_ref_count == 0) {
			NvRmTransportDeInit(s_hRmGlobal);
		}
...
static int nvrpc_ioctl_init(struct file *flip,
			    unsigned int cmd, void __user *arg)
{
	int error;
	NvError e = NvSuccess;
	struct nvrpc_handle_param op;
	int *pMem = (int *)flip->private_data;
...
static int nvrpc_ioctl_deinit(struct file *flip,
			    unsigned int cmd, void __user *arg)
{
	int *pMem = (int *)flip->private_data;
				 ...
				 unsigned int cmd, unsigned long arg)
{
	...
	case NVRPC_IOCTL_INIT:
		err = nvrpc_ioctl_init(file, cmd, uarg);
		break;
	case NVRPC_IOCTL_DEINIT:
		err = nvrpc_ioctl_deinit(file, cmd, uarg);
		break;
