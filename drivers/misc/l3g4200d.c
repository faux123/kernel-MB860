...
/** Data returned from WHO_AM_I register   */
#define L3G4200D_ID_VALUE		0x34
...
{
	input_report_abs(gyro->input_dev, ABS_TILT_X, xyz[0]);
	input_report_abs(gyro->input_dev, ABS_TILT_Y, xyz[1]);
	input_report_abs(gyro->input_dev, ABS_GAS, xyz[2]);
	if (l3g4200d_debug)
...
static int l3g4200d_input_init(struct l3g4200d_data *gyro)
{
	...
	input_set_abs_params(gyro->input_dev, ABS_TILT_X, -G_MAX, G_MAX, FUZZ, FLAT);
	input_set_abs_params(gyro->input_dev, ABS_TILT_Y, -G_MAX, G_MAX, FUZZ, FLAT);
	input_set_abs_params(gyro->input_dev, ABS_GAS, -G_MAX, G_MAX, FUZZ, FLAT);
...
static int l3g4200d_detect_device(struct l3g4200d_data *gyro) {
	u8 i2c_buffer;
	int err;

	i2c_buffer = L3G4200D_WHO_AM_I;
	err = l3g4200d_i2c_read(gyro, &i2c_buffer, 1);
	if (err < 0)
		return err;

	if (i2c_buffer == L3G4200D_ID_VALUE)
		return 0;
			   ...
			   const struct i2c_device_id *id)
{
	...
	if ( l3g4200d_detect_device(gyro) < 0){
		dev_err(&client->dev,
			"No L3G4200D device found on I2C bus-%d with address 0x%x\n",
