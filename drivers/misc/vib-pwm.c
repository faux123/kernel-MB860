...
static struct vib_pwm_data *pwm_misc_data;

static void vib_pwm_set(int on)
{
	if (on) {
		if (pwm_misc_data->pdata->power_on && !pwm_misc_data->vib_power_state) {
			pwm_misc_data->pdata->power_on(pwm_misc_data->pdata->freq, pwm_misc_data->pdata->duty_cycle);
			pwm_misc_data->vib_power_state = 1;
		}
		gpio_set_value(pwm_misc_data->pdata->gpio,
                                      pwm_misc_data->pdata->active_low ? 0 : 1);
	} else {
                gpio_set_value(pwm_misc_data->pdata->gpio,
                                      pwm_misc_data->pdata->active_low ? 1 : 0);

		if (pwm_misc_data->pdata->power_off && pwm_misc_data->vib_power_state) {
			pwm_misc_data->pdata->power_off();
			pwm_misc_data->vib_power_state = 0;
		}
	}
}
...
{
	vib_pwm_enable(&pwm_misc_data->dev, value);
}
...
static int vib_pwm_probe(struct platform_device *pdev)
{
	...
	pwm_data->vib_power_state = 0;
	pwm_data->vib_state = 0;
	...
	if (ret < 0) {
        ...
        gpio_direction_output(pwm_data->pdata->gpio,
                              pwm_data->pdata->active_low);
        gpio_set_value(pwm_data->pdata->gpio,
                              pwm_data->pdata->active_low);

	pwm_misc_data = pwm_data;
