...
static void vib_gpio_set(int on)
{
	if (on) {
		...
		gpio_set_value(misc_data->pdata->gpio,
...
{
	mutex_lock(&misc_data->update_mutex);
	vib_gpio_set(misc_data->vib_state);
	mutex_unlock(&misc_data->update_mutex);
}
